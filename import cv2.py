...
df_train = df.drop(df[df['class'] == 'disgust'].index)
df_val = df.drop(df[df['class'] == 'disgust'].index)
df_test = df.drop(df[df['class'] == 'disgust'].index)

# modify the constant CLASSES after dropping 'disgust' class.
...
image = cv2.imread('data/train/sad/S011_002_00000020.jpg',cv2.IMREAD_GRAYSCALE)
